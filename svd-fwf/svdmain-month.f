...
c grid point: NL for SST; NR for Tox
      ...
      parameter (iyst = 1979, iyend = 2008, mmax = 12)

	PARAMETER(NK=20,NL=4425,NR=4425,NT=30, nr2=nr   )
	parameter (m1=4425,m2=4425,np =1, nt_total=360    )
	...
	dimension ww(nx*ny)
        ...
        DIMENSION sst_total(NT_total,NL),tox_total(NT_total,NR)
        DIMENSION tox0(NT,NR2),toy0(NT,NR2)
...
c..........................................................
      ...
      DIMENSION toxtox(nx,ny,nk),toytoy(nx,ny,nk)

      character*4 varnam(4)
      character*16 mname(4)
      character*20 nvert

      open(unit=6,file='svd.out-12',form='formatted')
c////////////////////////////////////////////////////////////////////////

c	read(89,88) lsi
 88     format(60i1) 
 137    format(1x,i2,1x,17(i2,1x))
...
c
c       read in hh anomaly
c
      open(20,file='data/ersst_anom.1979-2008.mgrid.dat',
     &     form='unformatted',access='direct',recl=imt0*jmt0)


      do iy = iyst, iyend      ! 1979-2008

       do m = 1, mmax

 	 nrec1 = m + (iy-1979)*12
	 read(20,rec=nrec1) ((odata(i,j),i=1,imt0),j=1,jmt0)
         
	 kk=m+(iy-1979)*12
	 write(*,*) nrec1, iy,m,nrec1, odata(17,15), kk
...
c       read in Tox anomaly

      open(20,file='data/fwf_anom.1979-2008.kl.dat',
...
c ----------------------------------------
      ...
      do it=1,nt
      do i =1,nl
      sst(it,i)=sst_total( month+(it-1)*12, i)
      end do
      end do

      do it=1,nt
      do i=1, nr
      tox(it,i)=tox_total( month+(it-1)*12, i)
      end do
      end do
...
c	no_nomal = 0
 	no_nomal = 1
	if(no_nomal.eq.1) goto 892

c/////////////////////////////////////////////////////////////////////////

c   zhang: normalized !    there are problems when normalizing !
c    zhang-sdv of sst  0.4490628

	do j=1,nr
	DEVIZ(j)=0.0
	enddo

        CALL NORMAL(sst,NL,NT,ZAVE,DEVIZ)
	aa=0.0
        do j=1,nl
	...
	DEVIZ(j)=0.0
	enddo
        CALL NORMAL(tox0,NR2,NT,ZAVE,DEVIZ)
	aa=0.0
        do j=1,nr 
        aa=aa+DEVIZ(j)/float(nr2)
	enddo
	tox_d(month)=aa
	write(*,*)"zhang-sdv of sst",month, sst_d(month),aa
...
c...................................................................
c write out the two maps....


       nwrite= 540+ month
c      write(nwrite) COVAZS  ,SIGVET2 ,W,sdt1,sdt2,ak,bk
       write(nwrite) COVAZS20,SIGVET20,W,sdt1,sdt2,ak,bk

c      write(541   ) COVAZS,SIGVET2,W,sdt1,sdt2,ak,bk
